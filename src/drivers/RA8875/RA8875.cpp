...
{
  if (this->readReg(0) != 0x75) { return false; }
...
{
  if (layer >= layers::LAYER_END) { return; }
...
void RA8875::drawPixels(
  const Point & startPoint, const uint16_t count, const Colors color) const
{
  ...
  while (num--) { this->writeData((uint16_t)color); }
}

void RA8875::drawPixels(
  const Point & startPoint, const uint16_t count, const Colors * const color) const
{
  const uint16_t x        = startPoint.getX();
  const uint16_t y        = startPoint.getY();
  uint16_t num            = count;
  ...
  while (num--) { this->writeData((uint16_t)*ptrColor++); }
}

void RA8875::fillScreen(const Colors color) const
{
  this->draw(
    Rectangle({ 0, 0 }, this->getWigth() - 1, this->getHeight() - 1, color), true);
}
...
void RA8875::draw(const Ellipse & object, bool fill) const
{
  ...
  if (w != h)
  {
    this->ellipseHelper(object.getCenterW(), object.getCenterH(), w / 2, h / 2,
      (uint16_t)object.getColor(), fill);
  }
  else
  {
    this->circleHelper(object.getCenterW(), object.getCenterH(), w / 2,
      (uint16_t)object.getColor(), fill);
  }
}
...
{
  this->curveHelper(object.getCenterW(), object.getCenterH(), object.getWidth(),
    object.getHeight(), (uint8_t)object.getMode(), (uint16_t)object.getColor(),
    fill);
}
...
{
  this->triangleHelper(object[0].getX(), object[0].getY(), object[1].getX(),
    object[1].getY(), object[2].getX(), object[2].getY(),
    (uint16_t)object.getColor(), fill);
}
...
const VA::Colors RA8875::Colour565To332(const VA::Colors color)
{
...
{
  if (this->_size == displaySizes::_480x80 ||
      this->_size == displaySizes::_480x128 || this->_size == displaySizes::_480x272)
  {
...
void RA8875::GPIOX(bool status) const { this->writeReg(RA8875_GPIOX, status); }
...
void RA8875::circleHelper(
  uint16_t x, uint16_t y, uint16_t r, uint16_t color, bool filled) const
{
  /* Set X */
  ...
  this->writeData(
    RA8875_DCR_CIRCLE_START | (filled ? RA8875_DCR_FILL : RA8875_DCR_NOFILL));

  /* Wait for the command to finish */
  this->waitPoll(RA8875_DCR, RA8875_DCR_CIRCLE_STATUS);
}

void RA8875::rectHelper(
  uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, bool filled) const
{
  /* Set X */
  ...
  uint16_t r, uint16_t color, bool filled) const
{
  ...
  const uint16_t rx = calcR(r, w);
  const uint16_t ry = calcR(r, h);
...
void RA8875::touchEnable(bool on) const
{
  ...
  if (on)
  {
    ...
    this->writeReg(RA8875_TPCR0,
      RA8875_TPCR0_ENABLE | RA8875_TPCR0_WAIT_4096CLK | RA8875_TPCR0_WAKEENABLE |
        adcClk); // 10mhz max!
...
inline void RA8875::delay(volatile uint32_t micros) const
{
  while (micros--) continue;
}
...
uint16_t RA8875::readStatus(void) const { return *(LCD_REG); }
...
bool RA8875::waitPoll(uint16_t regname, uint16_t waitflag) const
{
  while (true)
  {
    ...
    if ((temp & waitflag) == false) { return true; }
  }
  return false;
}
