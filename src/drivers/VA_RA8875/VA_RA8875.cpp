...
void RA8875::drawPixels(const Point & startPoint, const uint16_t count, const Colors * const color) const
{
  ...
  while (num--)
  {
    this->writeData((uint16_t)this->Color565To332(*ptrColor++));
  }
}
...
void RA8875::setColor(uint16_t color) const
{
...
void RA8875::circleHelper(uint16_t x, uint16_t y, uint16_t r, uint16_t color, bool filled) const
{
  /* Set X */
...
void RA8875::roundRectHelper(
  uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, uint16_t color, bool filled) const
{
