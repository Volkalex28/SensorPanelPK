...
public:
  ...
  virtual void drawPixels(const Point & startPoint, const uint16_t count, const Colors color) const = 0;
  virtual void drawPixels(const Point & startPoint, const uint16_t count, const Colors * const color) const = 0;

  virtual void fillScreen(const Colors color) const = 0;
  virtual void draw(const Rectangle & object, bool fill = true) const = 0;
  virtual void draw(const Ellipse & object, bool fill = true) const = 0;
  virtual void draw(const Curve & object, bool fill = true) const = 0;
  virtual void draw(const RoundRect & object, bool fill = true) const = 0;
  virtual void draw(const Triangle & object, bool fill = true) const = 0;

  virtual void setColor(uint32_t color) const = 0;
