...
Rectangle::Rectangle(const Rectangle & other)
  : Rectangle(
      other.getNullPoint(), other.getWidth(), other.getHeight(), other.getColor())
{ /* There's nothing here */
}
...
const Point Ellipse::calcNullPoint(
  const Point & center, const uint16_t width, const uint16_t height)
{
...
Ellipse::Ellipse(const Point & center, const uint16_t width, const uint16_t height,
  const Colors color)
...
{
  this->setNullPoint(
    this->calcNullPoint(value, this->getWidth(), this->getHeight()));
}
...
{
  if (mode < Mode::MODE_END) { this->_mode = mode; }
}
...
const uint16_t RoundRect::getRadius(void) const { return this->_radius; }

void RoundRect::setRadius(const uint16_t value) { this->_radius = value; }
...
const std::vector<Point> & Triangle::getPoints(void) { return this->_points; }
