...
public:
	Adafruit_RA8875(enum RA8875sizes s);
	bool begin();
	void softReset(void);
	...
	void CharFont(uint8_t pos, uint8_t* data);
	void DownloadFont(uint8_t* buffer, uint16_t begin, uint16_t end);

	/* Graphics functions */
	void graphicsMode(void);
	uint8_t Colour565To332(uint16_t colour);
	...
	/* Layers */
	void useLayers(bool on);
	void layerEffect(RA8875Layers efx);
	void writeTo(RA8875Layers d);
	...
	void writeCommand(uint16_t d);
	uint16_t readStatus(void);
	bool waitPoll(uint16_t r, uint16_t f);
	uint16_t width(void);
	...
	*/
	...
	virtual size_t write(const uint8_t* buffer, size_t size) {
		this->textWrite((const char *)buffer, size);
		return size;
	}
#endif
...
private:
	...
	void curveHelper(int16_t xCenter, int16_t yCenter, int16_t longAxis, int16_t shortAxis, uint8_t curvePart,
